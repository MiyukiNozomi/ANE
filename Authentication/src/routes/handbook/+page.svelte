...
<div class="min-h-screen bg-gray-900 relative pb-2">
  ...
  >
    ...
    <h1 class="font-kumbh hidden md:block text-white text-4xl font-extralight">
  ...
  >
    ...
    <p>
      ...
      related to the actual processing of a request; other API response codes
      such as 4xx indicate either missing JSON fields or incorrect JSON field
      types.
    </p>
    <p>
      Stumbling upon any 5xx error code indicates a Bug. Please report such
      occurrences with the payload and endpoint URL to the email <a
    ...
    <p>
      Your requests will be ignored for an hour and will always receive a 429
      response code.
    </p>
    <h1
    ...
    <p>
      When you sign in through the Authentication Page, you're secretly given a
      long hex string like this one:
    </p>
    <div class="p-4 py-2 bg-slate-900 text-center rounded-lg">
      <p>fabfa533d4f54a13b091e8ac4edb7934b8de749163f0461ab5d2d573cae79f76</p>
    </div>
    <p>
      This "Root Key" is really, really important, it essentially holds the
      permission to change anything of your profile.
      <br />
      So let's say you want to log into a Chat application of some sort, you definitely
      don't want the application to have the permission to potentially re-setup 2FA
      without your consent.
    </p>
    <p>
      <strong><i> Technical note</i></strong>: This key lasts for 2 days,
      Regarding the authorization keys, they only last for a day. if you end up
      needing your root key, you can hit the 'Get Root Key' button inside of
      your account security page to get it.
    </p>
    <h2
      class="font-semibold text-2xl text-gray-100"
    ...
    <p>
      To summarize, the application requests to have a user identity given by
      sharing a secret with the server.
      <br />
      The server provides a short-lived authorization code that the user receives
      and either agrees or ignores.
      <br />
      The application is then given a "Authorization Session", it's basically given
      a long hex number that has no permission to mutate anything.
    </p>
    <p>
      Now, all the application can do is use your profile to know what is yours
      and what isn't. No funny business.
    </p>
    <h1
      class="font-semibold text-4xl text-white"
      id="sessions-and-authorizations"
    ...
    <p>
      Blue pathnames (and without the 'X') do not require authentication. (see
      the Sessions and Authorizations section.)
    </p>
    <div class="flex flex-col gap-4">
      {#each endpoints as ed}
        ...
        >
          ...
          {#if ed.onError}
            <h3 class="font-bold">Response Payload Type (On error)</h3>
            <div class="p-4 py-2 bg-slate-900 w-fit rounded-lg text-sm">
    ...
    <p>
      Let's say our app named 'MSN Clone' generated a secret, named 'banana'... <strong
        ><i
          >(To be fair, you should use an UUID or something with a decent level
          of entropy as a secret, but for the sake of simplicity I will use a
          word)</i
        ></strong
      >
      <br />
      ...
      endpoint.
      <br />On success, the server will give us a 'request-code', this is what
      we will give to the user in a URL of the following format:
    </p>
    <div class="p-4 py-2 bg-slate-900 text-center rounded-lg">
      <p>https://auth.ane.jp.net/sign/authorize/[request-code]</p>
    </div>
    <p>
      And now we wait!.. by requesting /api/authorizations/get-status every 2s
      using our shared secret until it either tells us the request expired, or
      the user authorized it.
    </p>
    <p>
      And that's it! you can now request /api/signed/me using the given session
      token!
    </p>
    <p>
    ...
    <p>
      ...
      they're stored in the <a
        href="https://developers.google.com/speed/webp"
        class="text-red-500 underline">WebP &UpperRightArrow;</a
      > format. They're are handled by this microservice on the following path:
    </p>
    <div class="p-4 py-2 bg-slate-900 text-center rounded-lg">

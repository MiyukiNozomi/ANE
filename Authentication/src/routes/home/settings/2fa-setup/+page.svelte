<script lang="ts">
  ...
  async function verify() {
    ...
    const apiRes = await progressAPI?.invokeAPIWithStatus<TwoFactorStepFinish>(
      "Verifying...",
      "signed/2fa/setup",
      { twoFactorCode }
    );
...
{#if recoveryKey}
  ...
  >
    ...
    >
      ...
      <p>Your account will now require a one time password from now on!</p>
      <p>
        To ensure you will never loose this account, store this recovery key
        somewhere safe in your computer.
      </p>
  ...
  <div class="flex flex-col gap-4 py-4 text-white p-8">
    ...
    <div class="flex flex-col gap-2">
      <p class="text-gray-200">
        Scan the QR code below using your authenticator app, or copy the shared
        secret into your one-time-password app.
      </p>
          ...
          <div class="flex flex-row gap-4">
            ...
            <button
              class="{progressAPI?.isActive()
                ? 'bg-blue-950'
                : 'bg-blue-600'} w-fit rounded-md px-4 py-2"

...
export namespace Backend {
  ...
  export async function isAlive() {
    try {
      const checkRes = await fetch("http://localhost:4050/is-alive", {
        method: "get",
      });
      if (checkRes.status != 200) return false;
    } catch (e) {
      return false;
    }
    return true;
  ...
  export async function getAccount(username?: string, accountId?: number) {
    if (username && accountId != null && accountId != undefined) {
      throw "Only one value allowed. BAD IMPLEMENTATION";
    }

    return await doAPIEndpoint<AccountInfo>(
      "/get-account",
  ...
  export async function getAuthorizationStatus(
    secret?: string,
    authRequestCode?: string
  ) {
    if (!secret && !authRequestCode) {
      throw "Only one value allowed. BAD IMPLEMENTATION";
    }

    return await doAPIEndpoint<AuthorizationStatus>(
  ...
  export async function register(username: string, password: string) {
    ...
    return await doAPIEndpoint<UserSessionInfo>("/register", {
      username,
      password,
    });
  }
  ...
  export async function login(
    ...
    return await doAPIEndpoint<UserSessionInfo>(
      "/login",
      twoFactorCode
        ? { username, password, "twofactor-code": twoFactorCode }
        : { username, password }
    );
  }
  /**
  ...
  export async function twoFactorEnableStep1(authToken: string) {
    return await doAPIEndpoint<TwoFactorStep1Start>(
      "/signed/2fa-enable/step1",
      undefined,
      authToken
    );
  }

  export async function twoFactorEnableFinalStep(
    twofactorCode: string,
    authToken: string
  ) {
    return await doAPIEndpoint<TwoFactorStepFinish>(
      "/signed/2fa-enable/setup",
      { "twofactor-code": twofactorCode },
      authToken
    );
  }

  export async function disable2FASupport(
    recoveryKey: string,
    authToken: string
  ) {
    return await doAPIEndpoint<unknown>(
      "/signed/2fa-disable",
  ...
  export async function createAPIToken(tokenName: string, authToken: string) {
    return await doAPIEndpoint<SessionInfo>(
  ...
  export async function getSecurityInfo(authToken: string) {
    return await doAPIEndpoint<AccountSecurityInfo>(
      "/signed/get-security-info",
      undefined,
      authToken
    );
  }
  ...
  export async function setDisplayName(displayname: string, authToken: string) {
    return await doAPIEndpoint<any>(
  ...
  export async function getSessions(authToken: string) {
    return await doAPIEndpoint<GetSessionsInfo>(
      "/signed/get-sessions",
      undefined,
      authToken
    );
  ...
  async function doAPIEndpoint<T>(
    endpoint: string,
    payload: any,
    authToken?: string
    ...
    try {
      ...
      if (response.status != 200) {
        console.error(
          "API gave " + response.status + " which is unsupported. "
        );
